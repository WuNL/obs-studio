...
class ByteWebRTCStreamInterface
...
class ByteWebRTCStream
	: public rtc::RefCountedObject<ByteWebRTCStreamInterface> {
public:
	enum Type {
		ByteWebRTC = 0,
	};

	ByteWebRTCStream(obs_output_t *output);
	~ByteWebRTCStream() override;
...
private:
	...
	void onOpened(const char *sdp);
	//size_t readHeader(char *header, size_t size, size_t nitems,
	//		  void *userdata);
